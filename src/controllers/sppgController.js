...
const sppgController = {
    ...
    async updateProfile(req, res) {
        try {
            const id_user = req.user.id_user;
            const { sppg_name, sppg_address } = req.body;
    ...
    async getDashboardData(req, res) {
        try {
            ...
            currentMonthReports.forEach(report => {
                totalPengeluaranBulanIni += Number(report.total_expense || 0);
            });
    ...
    async getPeriodicReports(req, res) {
        try {
            ...
            reports.forEach(report => {
                total_budget_period += Number(report.total_expense || 0);
            });
    ...
    async createDailyReport(req, res) {
        ...
        try {
            ...
            if (budgets) {
                parsedBudgets = typeof budgets === 'string' ? JSON.parse(budgets) : budgets;
                
                if (parsedBudgets.length > 0) {
                    parsedBudgets.forEach(b => {
                        grandTotal += Number(b.item_price);
                    });
                }
            }
